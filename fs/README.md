...
[include](./writeFile.js)

在上面的例子中，第3行尝试以「追加模式」打开一个文件，然后写入数据， 传递的缓冲区包含以下一些信息：
...
* 写入操作结束后被调用的回调函数 wrote

本例中，向文件中写入数据的起始位置是 null，表明写入操作将从当前文件的游标处开始。由于是以「追加模式」打开文件的， 因此此时文件的游标位于文件的结尾处。  
再次提醒，和读取操作一样，在 `fs.write` 函数执行之后请不要使用提供的缓冲区。一旦将其传递给 `fs.write` 函数， 缓冲区就处于写入操作的控制之下， 直到回调函数被调用之后才能重新使用它。

## 关闭文件

你也许已经注意到了，到目前为止，在本章见过的所有例子中都没有关闭文件的代码，这是因为它们都是只需要运行一次的简单小例子。当 Node 进程退出之后， 操作系统会确保所有文件都被关闭。

[include](./closeFile.js)

然而，在实际的应用程序中， 一旦打开一个文件，最后就必须关闭它。为此， 必须跟踪那些已经打开的「文件描述符」，在最后不再需要它们的时候就使用 `fs.close(fd[,callback)` 关闭它们。如果不细心的话， 很容易就会遗漏某个文件描述符。下面是一个例子，在这个例子中提供了一个叫 `openAndWriteToSystemLog` 的函数，该函数具有关闭文件的功能：
在上面的代码中，提供了一个名为 `openAndWriteToSystemLog` 的函数，它接受一个缓冲区用于写入数据，它还接受了一个回调函数，这个回调函数在操作结束或出现错误时被调用。如果发生了错误，错误对象就会被传递给回调函数，作为其第一个参数。

注意内部函数 `notifyError`，它会关闭文件描述符并报告发生的错误。

> **\[info\] 注意：**
>
> 到目前为止，你已经学习了使用底层的原语操作来打开、读写和关闭文件，然而 Node 还有一组更高级的构造函数， 允许以更简单的方式处理文件。
>
> 例如，也许你想以一种安全的方式，让两个或者多个 write 操作并发地向文件中追加数据此时应该使用 WriteStream。还有，当想读取文件中的某个区域时，可以考虑使用ReadStream。 以上两个用例会在[ 第9章 “读写数据流”](/stream/README.md)中介绍。

## 本章小结

在处理文件时，绝大多数时候都需要处理和提取文件的路径。 通过使用 path 模块，可以连接路径、 规范化路径、 计算路径之间的差别，以及将相对路径转换成绝对路径。 还可以提取指定文件路径的扩展名、 文件名和目录等路径的组成部分。

Node  在 fs 模块中提供了一组底层 API 来访问文件系统，底层文件系统使用「文件描述符」来处理它们对应的文件。可以用 `fs.open` 函数打开文件、用 `fs.write` 函数写入文件、用 `fs.read`  函数读取文件以及用 `fs.close` 函数关闭文件。

当出现错误时， 应该总是通过对错误进行正确的处理来关闭文件一一此时应确保在调用返回之前关闭所有的文件描述符。